...
export function ListToolbar({ query, onQueryChange, sort, onSortChange, direction, onDirectionChange, options, resultCount, searchLabel }) {
  return <div className="mb-5 flex items-end gap-3 rounded-xl border border-border bg-card p-3 shadow-[0_1px_2px_rgb(2_40_88_/_3%)] max-[700px]:grid max-[700px]:grid-cols-[1fr_auto]">
    ...
    <div className="w-48 max-[700px]:w-full"><Label className="sr-only" htmlFor="list-sort">Sort by</Label><Select value={sort} onValueChange={onSortChange}><SelectTrigger id="list-sort" className="h-10 w-full bg-white"><SelectValue /></SelectTrigger><SelectContent>{options.map((option) => <SelectItem key={option.value} value={option.value}>{option.label}</SelectItem>)}</SelectContent></Select></div>
    <Button className="size-10" variant="outline" size="icon" type="button" onClick={() => onDirectionChange(direction === 'asc' ? 'desc' : 'asc')} aria-label={`Sort ${direction === 'asc' ? 'descending' : 'ascending'}`}>{direction === 'asc' ? <ArrowUpAZ /> : <ArrowDownAZ />}</Button>
    <span className="sr-only" role="status" aria-live="polite">{resultCount} results</span>
  </div>
}
