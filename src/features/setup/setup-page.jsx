...
export function SetupPage() {
  ...
  return (
    <div className="dashboard setup-dashboard">
      ...
      {readiness.isSuccess && !readiness.data.ready && <section className="readiness-panel" aria-label="Setup progress"><div className="readiness-summary"><div><span>Setup progress</span><strong>{readiness.data.completedSteps} of {readiness.data.totalSteps} complete</strong></div><div className="readiness-meter" aria-label={`${readiness.data.completedSteps} of ${readiness.data.totalSteps} steps complete`}><span style={{ width: `${readiness.data.completedSteps / readiness.data.totalSteps * 100}%` }} /></div></div><div className="readiness-steps">{readiness.data.steps.map((step) => <button key={step.key} className={step.complete ? 'complete' : ''} type="button" onClick={() => setType(tabByReadinessKey[step.key])}><span>{step.complete ? <Check size={13} /> : step.count}</span>{step.label}</button>)}</div></section>}
