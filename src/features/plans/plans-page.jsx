...
import { approvePlan, completePlanStep, createPlanProposal, dismissPlan, planQueryOptions, plansQueryOptions, submitPlanChange } from '@/features/plans/plans-api.js'
...
function CreatePlanDialog({ activePlans, onClose }) {
  ...
  return <Dialog open onOpenChange={(open) => !open && onClose()}><DialogContent className="max-w-[560px] p-6"><DialogHeader><DialogTitle>Buat usulan rencana</DialogTitle><DialogDescription>Pilih batch aktif dan tujuan pengirimannya.</DialogDescription></DialogHeader>
    ...
    {eligible.length > 0 && <fieldset className="grid max-h-80 gap-2 overflow-y-auto"><legend className="sr-only">Batch yang memenuhi syarat</legend>{eligible.map((batch) => <Label className="flex cursor-pointer items-start gap-3 rounded-lg border border-border p-3 transition-colors hover:bg-slate-50 has-checked:border-primary/40 has-checked:bg-primary/5 focus-within:ring-3 focus-within:ring-ring/30" key={batch.id}><input className="mt-1 size-4 accent-primary" type="checkbox" checked={selected.includes(batch.id)} onChange={() => toggle(batch.id)} /><span><strong className="block text-sm">{batch.code}</strong><span className="text-xs text-muted-foreground">{batch.weightKg.toLocaleString('id-ID')} kg · Mutu {batch.grade} · {batch.location.name}</span></span></Label>)}</fieldset>}
...
function ReasonContent({ step }) {
  return <div className="grid gap-4"><div><p className="text-xs font-bold text-foreground">Insight AI</p><p className="mt-1 text-sm leading-relaxed text-muted-foreground">{step.rationale ?? 'Tidak ada insight tindakan yang tersimpan untuk langkah historis ini.'}</p></div><div><p className="text-xs font-bold text-foreground">Dasar waktu deterministik</p><p className="mt-1 text-sm leading-relaxed text-muted-foreground">{step.timingRationale ?? 'Tidak ada dasar waktu yang tersimpan untuk langkah historis ini.'}</p></div>{step.latestSafeAt && <div className="rounded-lg bg-muted px-3 py-2"><p className="text-xs font-bold text-foreground">Batas waktu aman terakhir</p><p className="mt-1 text-sm text-muted-foreground">{formatDateTime(step.latestSafeAt)}</p></div>}</div>
}
...
function ReviewActions({ plan }) {
  ...
  return <section className="rounded-xl border border-sky-200 bg-sky-50/50 p-5"><h2 className="text-sm font-bold text-sky-950">Tinjau usulan</h2><p className="mt-1 text-xs text-sky-900">Persetujuan mengaktifkan rencana ini untuk batch terkait. Penolakan menyimpannya dalam riwayat.</p>{mutation.isError && <div className="mt-3"><ErrorMessage error={mutation.error} /></div>}<div className="mt-4 flex gap-2 max-[480px]:flex-col"><Button disabled={mutation.isPending} onClick={() => mutation.mutate('approve')}>Setujui usulan</Button><Button variant="destructive-outline" disabled={mutation.isPending} onClick={() => mutation.mutate('dismiss')}>Tolak</Button></div></section>
}
...
function RevisionForm({ plan }) {
  ...
  const mutation = useMutation({ mutationFn: () => submitPlanChange(plan.id, instruction), onSuccess: async (result) => { await Promise.all([invalidatePlanQueries(queryClient), queryClient.invalidateQueries({ queryKey: ['resources'] })]); if (result.generation.status === 'PROPOSAL') navigate(`/plans/${result.generation.proposal.id}`) } })
  const description = plan.status === 'ACTIVE'
    ? 'Laporkan fakta operasional atau berikan preferensi untuk langkah mendatang. Laporan yang lengkap langsung memperbarui kondisi operasi; rencana aktif tetap tidak berubah hingga usulan pengganti disetujui.'
    : 'Laporkan fakta operasional atau berikan preferensi untuk mengganti usulan ini.'
  const noProposal = mutation.data?.generation.status === 'NO_VALID_PROPOSAL_FOUND' ? mutation.data.generation : null
  return <section className="rounded-xl border border-border bg-card p-5"><h2 className="text-sm font-bold">Laporkan perubahan atau revisi rencana</h2><p className="mt-1 text-xs leading-relaxed text-muted-foreground">{description}</p><Label className="sr-only" htmlFor="revision-instruction">Perubahan operasional atau preferensi revisi</Label><Textarea id="revision-instruction" className="mt-4 min-h-28" value={instruction} maxLength={2000} placeholder="Contoh laporan: TR-01 tidak tersedia. Contoh preferensi: Kirim B-017 besok pagi." onChange={(event) => setInstruction(event.target.value)} />{mutation.isError && <div className="mt-3"><ErrorMessage error={mutation.error} /></div>}{mutation.data?.kind === 'REPORT_APPLIED' && <p className="mt-3 rounded-lg border border-blue-200 bg-blue-50 p-3 text-sm text-blue-900" role="status"><strong className="block">Laporan operasional diterapkan</strong>{mutation.data.report.entityName} → {String(mutation.data.report.value)}</p>}{noProposal && <p className="mt-3 rounded-lg border border-amber-200 bg-amber-50 p-3 text-sm text-amber-800" role="status"><strong className="block">Tidak ditemukan usulan revisi yang valid</strong>{noProposal.reason}{mutation.data.kind === 'REPORT_APPLIED' && <span className="mt-1 block">Laporan tetap tersimpan sebagai fakta operasional.</span>}</p>}<Button className="mt-3" disabled={!instruction.trim() || mutation.isPending} onClick={() => mutation.mutate()}>{mutation.isPending ? 'Memproses perubahan…' : 'Proses perubahan'}</Button></section>
}
