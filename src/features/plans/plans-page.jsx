...
function CreatePlanDialog({ activePlans, onClose }) {
  ...
  const mutation = useMutation({ mutationFn: createPlanProposal, onSuccess: async (result) => { if (result.status === 'INFEASIBLE') return; await invalidatePlanQueries(queryClient); onClose(); navigate(`/plans/${result.proposal.id}`) } })
...
function PlanSteps({ plan, completion }) {
  ...
  return <ol className="divide-y divide-border">{steps.map((step, index) => {
    ...
    return <li className={cn('grid grid-cols-[40px_1fr_auto] items-center gap-3 p-5', (completed || canceled) && 'bg-slate-50 text-slate-500')} key={step.id}><span className={cn('grid size-8 place-items-center rounded-full border text-xs font-bold', completed ? 'border-primary bg-primary text-white' : 'border-slate-300 bg-white')}>{completed ? <Check size={15} /> : canceled ? <CircleX size={15} /> : index + 1}</span><div><strong className={cn('block text-sm', canceled && 'line-through')}>{describeStep(step)}</strong><span className="mt-1 block text-xs">{completed ? `Completed ${formatDateTime(step.completedAt)}` : canceled ? 'Canceled' : `Scheduled ${formatDateTime(step.scheduledAt)}`}</span>{step.notes && <span className="mt-1 block text-xs text-muted-foreground">{step.notes}</span>}</div>{completion && step.status === 'UPCOMING' && <Button size="sm" variant="outline" disabled={completion.isPending} onClick={() => completion.mutate({ planId: plan.id, stepId: step.id })}><CheckCircle2 />{marking ? 'Marking…' : 'Mark complete'}</Button>}</li>
  })}</ol>
}
...
function RevisionForm({ plan }) {
  ...
  const mutation = useMutation({ mutationFn: () => createPlanRevision(plan.id, instruction), onSuccess: async (result) => { if (result.status === 'INFEASIBLE') return; await invalidatePlanQueries(queryClient); navigate(`/plans/${result.proposal.id}`) } })
