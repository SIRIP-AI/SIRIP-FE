...
function ColdStorageCard({ resource, onEdit, onDelete }) {
  ...
  return (
    <article className={cardClass} data-animate-card>
      ...
      <div><div className="flex items-baseline gap-2"><strong className="text-xl tracking-[-.03em]">{resource.availableCapacityKg.toLocaleString('id-ID')} kg</strong><span className="text-xs text-muted-foreground">tersedia</span></div><div className="mt-3 h-1.5 overflow-hidden rounded-full bg-muted" role="progressbar" aria-label="Kapasitas terpakai" aria-valuemin="0" aria-valuemax="100" aria-valuenow={usedPercent}><span className="block h-full rounded-full bg-primary" style={{ width: `${usedPercent}%` }} /></div></div>
...
function TemperatureHistory({ readings, pending, error }) {
  ...
  if (error) return <div className="grid h-28 place-items-center rounded-lg bg-red-50 px-4 text-center text-xs font-medium text-red-700" role="status">Riwayat suhu tidak tersedia</div>
    ...
    <div className="mx-auto w-full max-w-[1180px] px-8 pt-12 pb-7 max-[780px]:px-4 max-[780px]:py-6">
      ...
      <Tabs value={type} onValueChange={setType}>
        <div className="mb-5 overflow-x-auto rounded-xl border border-border bg-card p-1">
          <TabsList className="grid h-11 min-w-[520px] w-full grid-cols-4 bg-transparent p-0" aria-label="Jenis sumber daya">
